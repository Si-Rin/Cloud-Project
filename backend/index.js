...
function factorialModulo(n, mod) {
  let result = 1n;
...
app.get('/stress', (req, res) => {
  const requestedSeconds = Number.parseInt(String(req.query.seconds || '15'), 10);
  const seconds = Math.min(Math.max(Number.isNaN(requestedSeconds) ? 15 : requestedSeconds, 1), 120);
  ...
  while (Date.now() < stopAt) {
    for (let i = 0; i < 150; i += 1) {
      const factorialInput = 700n + BigInt((cycles + i) % 250);
      checksum ^= factorialModulo(factorialInput, modulo);

      const primeCandidate = 5000003 + ((cycles * 150 + i) % 20000);
      if (isPrime(primeCandidate)) {
        checksum += BigInt(primeCandidate);
      }
    }
